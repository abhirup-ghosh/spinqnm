...
Hanford and Livingston, having identical PSDs \textcolor{red}{RB:did we use Virgo? here it seems not, but above we say we used Virgo}. The distance to the two 
synthetic events is rescaled such that the SNR in the detector network
is the same as the actual events (i.e., 24 for GW150914 and 14 \textcolor{red}{RB: $\rho_{\rm IMR}$ for the GW190521-like injection shown in the table is larger than this...}
...
deviation from the GR prediction \textcolor{red}{RB: not sure what this means, since a GR waveform by itself cannot detect a non-GR deviation}. However in the case of the
...
the two parameterized estimates: with and without deviations\textcolor{red}{RB: I don't understand this previous sentence. Discuss with Abhirup}.
...
place \textcolor{red}{RB:"in its place" is weird, since the (2,2) is always louder than the (3,3) mode (unless you're thinking about very high mass BHs for which the (2,2) mode would be out-of-band, but the (3,3) still in band. But in any case, I would finish the sentence at "has not been sufficient to measure it along with the $(2, 2)$"}. However, it might also be possible to combine information from
multiple observation over the coming few years to obtain meaningful
constraints on the $(3, 3)$ and other sub-dominant QNMs. \textcolor{red}{reminder: citations needed here}
