...
We \ab{now} demonstrate our method using synthetic-signal injections describing GWs
from BBHs in GR. We employ coloured Gaussian noise with PSDs for LIGO and
Virgo detectors \ab{during the fourth observing (O4) run} (i.e., at design sensitivity)\ab{, which 
...
detections \ab{of GW150914 and GW190521}, we consider a two-detector LIGO network at
...
red (GR template) and blue (non-GR template) \sout{diverge} \ab{differ}. We summarize the results of the Bayesian analysis in Fig.~\ref{fig:simulated_signal_nonGR} where we
...
\begin{figure}
        ...
        \caption{\textcolor{red}{FINAL RESULT} Top panel: The `+'--polarization of the gravitational waveform $h_+(t)$ from a GW150914-like event where the post-merger is described by GR (i.e., $\df{220} = \dtau{220} = 0$), and where the merger-ringdown is modified (i.e., $\df{220} = \dtau{220} = 0.1$). Bottom panel: Comparison of the evolution of the amplitude, $\tilde{h}(t)$ (left) \comment{AB: what is the amplitude $h$? How is it obtained from the polarizations?} and instantaneous frequency, $f(t)$ (right) for the GR and non-GR signal.}
...
all the physics in the $\pSEOB$ signal.} \comment{AB: I erased the previous sentence because 
$\pSEOB$ has not ``more'' physics. We varied the QNMs by 10\% with respect GR, we have a different gravity theory 
which we cannot match with a GR template. I don't think we need to give any explanation of the results, 
they are self-evident to me.} We also notice that the
...
Finally, we investigate the impact on the estimation of \ab{the} GR parameters
...
prediction, but quite plausible \ab{during O4.} \sout{at design sensitivities.} Using this
...
injection parameters, the predicted values of the $(2,\pm 2)$ and
$(3,\pm 3)$ frequency and damping time are \macro{(169.45 Hz, 4.68
  ms)} and \macro{(271.21 Hz, 4.50 ms)} respectively. The left panel
of Fig.~\ref{fig:nohair_sxs} shows that the 2D posteriors
on the $(2,\pm 2)$ and $(3,\pm 3)$ QNMs are consistent with the
...
final mass and spin for the $(2,\pm 2)$ (blue) and $(3,\pm 3)$ (red)
...
so far, the power in the $(3,\pm 3)$ has not been sufficient to
measure it along with the $(2,\pm 2)$, or in fact, in its
...
constraints on the $(3,\pm 3)$ and other sub-dominant QNMs.
