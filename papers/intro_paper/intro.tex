...
spectroscopy), and consistency between \ab{them~\cite{Dreyer:2003bv,Carullo:2018sfu,Carullo:2018gah,Bhagwat:2019bwv,Cabero:2019zyt,Maselli:2019mjd}}. The
...
complex QNM frequencies with our parameterized model within a Bayesian formalism\ab{, and validate it using 
several synthetic-signal injection studies in Gaussian noise}. Then, in Sec.~\ref{sec:results},
we \sout{demonstrate} \ab{apply}  our method on real GW events \ab{using LIGO and Virgo data}.\sout{, as well as simulated signals}. In particular, we analyze
the GWTC 1/2  events and \ab{obtain the most stringent constraints on the dominant (complex)} QNM frequecies \ab{with our method}\sout{to be \macro{$\df{220} = 0.02^{+0.03}_{-0.03}$} and \macro{$\dtau{220} = 0.11 ^{+0.12} _{-0.12}$}}. \comment{AB: I think it is too early to give the results here.} Finally, in Sec.~\ref{sec:discussion} we provide a summary of
