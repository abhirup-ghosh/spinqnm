\documentclass[twocolumn,prd,aps,superscriptaddress,preprintnumbers,tightenlines,showpacs,nofootinbib,amsfonts,amsmath,longbibliography]{revtex4-1}
\usepackage{epsfig}
\usepackage{graphics}
\usepackage{graphicx}
...
When possible, we also combine the marginalized posterior distribution on
$\ell$ for both event GW150914 and GW200129. To do so, we employ the same
approach as Ref.~\cite{Perkins:2021mhb} which relies in obtaining a kerneld
density estimator (KDE) as an approximation for the posterior distribution on
$\ell$ for each individual events which are then multiplied together to yield
the cumulative posterior.
%
...
%
\begin{equation}
P(\ell \leqslant \ell') = \int_{-\infty}^{\,\ell'} P(\ell') \, \dd \ell'\,.
\label{eq:def_cdf}
\end{equation}
%
...
\quad \textrm{and} \quad
\gamma \, \delta \tau^{(j)} \ll 1 \,.
\label{eq:parspec_bound}
\end{equation}
%
for all $j$ of the expansion in dimensionless spin $\chi$.
%
We will also construct posterior distributions on the parameters and check if
...
horizontal lines lay at 0.68 and 0.90.
}
\label{fig:dcs_cdf}
\end{figure}
...
deviation from GR. This is not the case. We must remember that in the {\sc
Parspec} formalism the GR deviations are controlled by $\gamma \, \delta
\omega^{(j)}$~and~$\gamma \, \delta \tau^{(j)}$, and these, indeed, do peak at
zero. (See Fig. xx discussed below.)
%
...
evaluates with Eq.~\eqref{eq:def_cdf}, with $\ell' = \Lambda_{\rm EFT}$,
...
with $\varepsilon \leq 1/2$, i.e., below the criteria used
...
%
\begin{equation}
\ell_{\rm CS} \leqslant 46.5~\textrm{km}~( = \Lambda_{\rm EFT}(\tfrac{1}{2}, M_{f}) )
\quad \textrm{at 90\% credibility},
\end{equation}
%
...
%
\begin{equation}
\ell_{\rm CS} \leqslant 27.4~\textrm{km}~( = \Lambda_{\rm EFT}(0.3, M_{f}) )
\quad \textrm{at 68\% credibility},
\end{equation}
%
can also be placed, when we include the $j=1$ QNM correction.

We can draw qualitatively similar conclusions from the GW200129 event.
In particular, we find,
%
\begin{equation}
\ell_{\rm CS} \leqslant 43.9~\textrm{km}~( = \Lambda_{\rm EFT}(\tfrac{1}{2}, M_{f}) )
\quad \textrm{at 90\% credibility},
\end{equation}
%
and
%
\begin{equation}
\ell_{\rm CS} \leqslant 19.3~\textrm{km}~( = \Lambda_{\rm EFT}(0.3, M_{f}) )
\quad \textrm{at 68\% credibility},
\end{equation}
%
when we include the $j=1$ QNM correction.

These slightly stronger bound are a consequence of the larger support for $\ell_{\rm CS} \lessapprox 15$~km
for GW200129 (compare the top and middle panels in Fig.~\ref{fig:dCS_exec_sum}), and in part due to
the smaller median remmant ($M_{f} \approx 59.5\msun$ versus $M_{f} \approx 61.8\msun$ for GW150914).
...
\caption{(Color online).~The cumulative distribution function evaluated at the cut-off $\Lambda_{\rm EFT}(\varepsilon, \gm)$
as a function of the parameter $\varepsilon$ for both $\gm = m_2$ (blue curves), the secondary's source mass,
and $\gm = M_{f}$ (orange curves), the remnant's source mass, without (solid curve) and with (dashed curves)
linear in spin QNM corrections. The horizontal lines lay at 0.68 and 0.90.
%
We see that the CDF at $\varepsilon = 1/2$ is approximately one when $\gm =
M_{f}$ for both events, with and without including the $j = 1$ dCS corrections
to the dominat QNM.
}
\label{fig:dcs_cdf}
\end{figure}
...
\hline
\hline
Event & $j$ & Eq.~\eqref{eq:eft_bound}? & Eq.~\eqref{eq:parspec_bound}?  & Bound \\
\hline
GW150914 & $j = 0$    & Yes & Yes & $\ell_{\rm CS} \leqslant \dots$~km \\
GW150914 & $j = 0, 1$ & Yes & No  & --                                 \\
GW200129 & $j = 0$    & Yes & Yes & $\ell_{\rm CS} \leqslant \dots$~km \\
GW200129 & $j = 0, 1$ & Yes & No  & --                                 \\
\hline
\hline
\end{tabular}
\caption{Detailed summary of our results for dCS gravity.
%
We assume $\varepsilon = 1/2$.}
\end{table}
