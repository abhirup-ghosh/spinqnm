...
%
The parameter $\Lambda_{\rm EFT}$ was used
Refs.~\cite{Nair:2019iur,Perkins:2021mhb,Lyu:2022gdr} with $\varepsilon = 1/2$,
but here we  $\varepsilon \in [0, 1]$.
%
We will say that a bound has been placed on $\ell$, if most of the support of
the posterior distribution function $P(\ell)$ is in the interval $[0, \, \Lambda_{\rm EFT}(\varepsilon; \, \mathfrak{m})]$.
%
...
%
Another question to consider is the following: what is the mass $\mathfrak{m}$ that we should use?
%
...
signal alone, it was natural to chose the secondary's mass $m_2$ as the most
...
%
Here we adopt an agnostic view and consider both masses, $m_2$ and $M_f$, to
...
threshold $T(\varepsilon;\, m)$ EdGB gravity as a function of the parameter
...
%
\begin{equation}
\ell_{\rm CS} \leqslant \Lambda_{\rm EFT}(1/2; \, M_{f}) = \dots \,.
...
%
These conclusions for GW150914 are shared with GW200129.
