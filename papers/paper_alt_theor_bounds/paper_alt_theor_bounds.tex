...
calculated on a theory-by-theory basis. In particular, we focus on theories that deviate
from general relativity by higher-order curvature corrections, namely,
...
constrained with these events, we can place upper bounds on the \sout{coupling
constants} \ab{fundamental length-scale} of cubic- ($\ell_{\rm cEFT} \leqslant 38.2$~km) and quartic-order
...
%
\ab{For completness,} in Appendix~\ref{app:big_contour} we also 
...
GW150914 and GW200129. \ab{Our results are summarzied in Table~\ref{tab:bound_summary}.}

\ab{In particular,} we found, that within the interpretation \ab{of} these theories as EFTs and the
region of validity of the ParSpec framework, the \sout{coupling constant} \ab{fundamental length-scale} of dCS gravity is bound as
...
%
In contrast, we could not place any bounds on the \sout{coupling constant} \ab{length-scale} \abcomm{Maybe 
this is common in the literature, but I would call it length-scale instead of coupling 
constant. What do you think?} of EdGB gravity $\ell_{\rm EdGB}$.
%
...
%
\ab{Using data of the LVK BBHs, it was found that the posterior distributions for 
...
\ab{Here, for the theories described in Sec.~\ref{sec:review_theories}, we 
...
of the polar gravitational-\sout{led}\ab{dominant} mode increases as the coupling constant
$\ell_{\rm EdGB}$ is increased. The leading-order spin corrections to the
branch of QNMs was calculated in Ref.~\cite{Pierini:2021jxd}.
%
Hence, according to the prescription of Sec.~\ref{sec:theory_specific_qnm},
this is the gravitational-led \abcomm{led or dominant?} branch of QNMs we select.

For the nonrotating QNMs we use\sout{d} \abcomm{We are using present tense when 
describing results in the paper, so I would suggest with stick with that consistently 
in the Appendix.} the numerical data of Ref.~\cite{Blazquez-Salcedo:2016enn} and
generate a new quartic fit in $\gamma_{\rm EdGB} = \ell_{\rm EdCS} / M$ using numerical QNM data valid for small
values of the coupling constant (see, in particular, Eq.~(27) and Fig.~2 of 
...
which for consistency with our previous discussion, we truncate at quartic-order in $\gamma_{\rm EdGB}$ \abcomm{$\ell_{\rm EdCS}^4$?}.
%
The resulting formula can then be expanded in $\gamma_{\rm EdGB}$ and the coefficients $\delta \omega^{(i)}$ and $\delta \tau^{(i)}$, $i=1,2$ \ab{can be}  
...
\ab{We show} in Fig.~\ref{fig:corner_plot_all} a corner plot \ab{for all the} intrinsic binary parameters from our parameter-estimation study of GW150914, using the \pSEOB{} waveform models for GR and dCS.
%
We see that \ab{the} median\ab{s} of the \ab{posterior distributions are} not affected substantially by the inclusion of the non-GR parameters. \ab{This is particularly important for the $\Mf$ and $\chi_{\rm f}$ parameters, since 
the ParSpec framework assumes that the non-GR theory induces small deviations from GR.} 
%
We \ab{obtain} qualitative similar results for GW200129 and the other modified gravity theories considered in our work.
%
