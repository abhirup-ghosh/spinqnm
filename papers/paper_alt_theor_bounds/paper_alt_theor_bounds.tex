...
Our result contrasts with those of Refs.~\cite{Nair:2019iur,Perkins:2021mhb,Lyu:2022gdr}
...
\caption{(Color online).~Similar to Fig.~\ref{fig:dCS_exec_sum}, but for dCS gravity.
%
...
In Fig.~\ref{fig:cEFT_cdf} we show the CDF for
both events, calculated as already described for the EdGB and dCS cases.
%
We see that curves are very similar to those of dCS gravity for GW200129 (cf.
bottom panel in Fig.~\ref{fig:cEFT_cdf}).
%
Moreover, we find that for both events both the EFT~\eqref{eq:eft_bound} and
ParSpec~\eqref{eq:parspec_bound} bounds are satisfied for $\mathfrak{m} = M_f$
and $N_{\rm max} = 1/2$, allowing us to obtain the combined bound
%
\begin{equation}
    \ell_{\rm cEFT} \leqslant 51.3~\textrm{km},
\end{equation}
%
...
the QNMs, implying that we cannot use the associated posterior distributions on
$\ell_{\rm cEFT}$ to draw any meaningful constraint on this parameter.
%
...
At last, let us consider the quartic EFT of GR.
%
...
inclusion of the spin corrections yields waveforms which for $\ell_{\rm qEFT} \gtrsim 50$~km
with ringdowns portions of the larger than the inspiral-plunge part \dots
%
To avoid this issue we lowered the value of $\ell^{\rm max}_{\rm qEFT}$, but at doing so we obtained which
were essentially uniform. Thus, we do not quote any results for $N_{\rm max} = 1$.
%
