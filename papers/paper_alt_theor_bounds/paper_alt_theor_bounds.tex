...
%
This is a critical different between our study and Ref.~\cite{Carullo:2021dui}.
In particular, as we will see in Sec.~\ref{sec:results}, adding theory-specific
information to the ParSpec expansion \emph{can lead to different interpretation
of the final results, even for different theories which predict modifications
with the exponent $p$.}
...
with black (purple) curve corresponding to $N_{\rm max} = 0$ ($N_{\rm max} = 1$) in
the spin expansion.
%
...
In Fig.~\ref{fig:cEFT_cdf} we show the cumulative distribution functions for
both events, calculated as done in EdGB and dCS cases.
%
We see that curves are very similar to those of dCS gravity for GW200129 (cf.
bottom panel in Fig.~\ref{fig:cEFT_cdf}).
%
We find that for both events both the EFT~\eqref{eq:eft_bound} and ParSpec~\eqref{eq:parspec_bound}
bounds are satisfied for $\mathfrak{m} = M_f$ and $N_{\rm max} = 1/2$, allowing us to obtain the
combined bound from both events
%
\begin{equation}
...
GW150914 (top panel), GW200129 (middle panel) and combined events (bottom panel).
%
In all panels, we use purple (black) lines correspond to the inclusion (or not) of the linear
in spin QNM correction.
}
