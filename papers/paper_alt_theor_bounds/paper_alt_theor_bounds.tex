...
defined in Eq.~\eqref{eq:def_gamma} as well as the sets, $\{\delta \omega_k^{(j)}\},$ and
$\{\delta \tau_k^{(j)}\}$, where $j$ ranges from zero to the power of spin in Eqs.~\eqref{eq:kerr_expansion}
up to which we include corrections.
%
...
%
It is important to emphasize that our procedure is different from that of Ref.~\cite{Carullo:2021dui}
...
outline the highlight results. Wherever possible, we also combine results from
...
\subsubsection{General remarks for the interpretation of our results}
\label{sec:remaks}

There are two requirements that we must take into account before
we can confidently claim to have placed a constraint, with our model assumptions,
with our parameter estimation study.

First, as described in Sec.~\ref{sec:review_theories}, all theories that we consider must be considered as an
effective field theory, meaning it has a sensible description only below an energy, or equivalently an length scale.
%
As a threshold for the validity of the EFT we use
%
\begin{equation}
T_{\rm EFT} (\varepsilon; \, \mathfrak{m}) = \varepsilon \,  (G \mathfrak{m} / c^2)\,,
\end{equation}
%
where $\varepsilon$ is a dimensionless number and $\mathfrak{m}$ the median value of one of the
masses involved in the problem.
%
We also momentarily restored factor of $c$ and $G$ to emphasize that this is a quantity
with dimensions of length and can hence be compared with the coupling strength $\ell$.
%
The parameter $T_{\rm EFT}$ was used in both
Refs.~\cite{Nair:2019iur,Perkins:2021mhb} with $\varepsilon = 1/2$,
but here we will assume $\varepsilon \in [0, 1]$.
%
We will say that a bound has been placed on $\ell$, if most of the support of
the posterior distribution function $P(\ell)$ is in the interval $[0, \, T_{\rm EFT}(\varepsilon; \, \mathfrak{m})]$.
%
In practice, this can be quantified by the cumulative distribution function
(CDF) associated to the marginalized posterior distribution $P(\ell)$,
%
\begin{equation}
P(\ell \leqslant x) = \int_{-\infty}^{x} P(x) \, \dd x\,.
\end{equation}
%
We will demand, for instance, that for a bound with 90\% credibility to be meaningfully placed that,
%
\begin{equation}
% \textrm{CDF}(\ell) \leqslant \textrm{CDF}(T_{\rm EFT}(\varepsilon; \, \mathfrak{m})).
P(\ell \leqslant T_{\rm EFT}) \leqslant 0.9 \,.
\end{equation}

%
Another question to consider is: what is the mass $\mathfrak{m}$ that we should use?
%
In Refs.~\cite{Nair:2019iur,Perkins:2021mhb}, which attempted to constrain dCS and shift-symmetric sGB theories with the \emph{inspiral}
signal alone, it was natural to chose the secondary's mass $m_2$ as the most conservative choice, since it is by definition the smallest mass
and hence places the lowest threshold for the validity of either theory as an EFT.
%
In our problem, the answer is not as clear. On the one hand, since we are
interested in the ringdown portion of the signal, it is natural to use the
final mass $M_f$ to compute the threshold $T_{\rm EFT}$.
%
On the other hand, one may argue that the modified gravity theory under study
should be be able to predict a full inspiral, merger, and ringdown of the black
hole binary, and thus the same most conservative $m_2$ should be used.
%
Here we take an agnostic approach and consider both masses, $m_2$ and $M_f$, to
establish $T_{\rm EFT}$, and we will compare how different assumptions yield
different interpretations of the results of our parameter estimations.

There is a second criteria we must also take into account. As already emphasized in Ref.~\cite{Maselli:2019mjd},
the {\sc Parspec} is by construction perturbative. This means that the non-GR deformation parameters are small,
i.e.,
%
\begin{equation}
\gamma \delta \omega^{(j)} \ll 1 \,, \quad \textrm{and} \quad \gamma \delta \tau^{(j)} \ll 1 \,.
\end{equation}
%
for all indices $j$ in the expansion in dimensionless spin $\chi$.
%
We will also construct posterior distributions on the parameters and check if most of their support
is constrained to be much less than unity.
...
\caption{(Color online).~The cumulative distribution function evaluated at the threshold $T(\varepsilon;\, m)$
as a function of the parameter $\varepsilon$ for both (blue curves) $m = m_2$, the secondary's source mass,
and (orange curves) $m = M_{f}$, the remnant's source mass, without (solid curve) and with (dashed curves)
linear in spin QNM corrections.
}
\label{fig:dcs_cdf}
\end{figure}
...
Serguei~Ossokine, and Helvi~Witek for discussions.
%
