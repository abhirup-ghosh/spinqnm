...
A general procedure to describe deviations from the Kerr QNM frequencies $\omega_{J}^{\rm K}$ and
damping times $\tau_{J}^{\rm K}$ is to write,
%
\begin{subequations}
...
\end{subequations}
%
where $\delta\omega_{J}$ and $\delta\tau_{J}$ are the deformation parameters.
%
Here we use the subscript $J$ to represent the QNM labels $\{l,\, m,\, n\}$,
...
A caveat of the parametrizations of the form~\eqref{eq:general_deviation}, is that
$\delta\omega_{J}$ and $\delta\tau_{J}$ depend, in general, on the BH's source mass and spin.
%
...
We remark that in the GR limit, where $\delta\omega_{J}^{(n)} = \delta\tau_{J}^{(n)} = 0$ $\forall\, n$,
...
%
The values of the fitting coefficients $\omega_{J}^{(n)}$ and $\tau_{J}^{(n)}$
...
+ \sum_{n=0}^{N_{\rm max}} \chi^{j} \tau^{j}\,,
\end{align}
\end{subequations}
%
We will work exclusively with the $\ell = m = 2$ and $n = 0$ (fundamental mode)
QNM in this work, that is, $J = \{2,\, 2,\, 0\}$, and we  have pulled out of
the sum all beyond-GR corrections, restricting ourselves to the nonspinning
($n=0$) and linear-order in spin ($n=1$) corrections to the Kerr QNMs.
%
These corrections are encoded in the coefficients $\delta\omega^{(j)}$ and $\delta\tau^{(j)}$.
