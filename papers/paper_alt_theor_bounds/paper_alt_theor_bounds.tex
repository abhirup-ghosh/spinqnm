...
formalism (ParSpec) introduced in Ref.~\cite{Maselli:2019mjd}, but following
some of the notation of Ref.~\cite{Carullo:2021dui}.
%
A general procedure to describe deviations from the Kerr quasinormal mode frequencies $\omega_{J}^{\rm K}$ and
damping time $\tau_{J}^{\rm K}$ is to write them as,
%
\begin{subequations}
...
\end{subequations}
%
as done e.g., in Refs.~\cite{Gossan:2011ha,Meidam:2014jpa,Carullo:2018sfu}.
%
Here we use $J$ to collectively represents the QNM labels $\{l,\, m,\, n\}$,
where $l$ and $m$ are the multipole indices, and $n$ the overtone number.
%
We work exclusively with $\ell = m = 2$ and $n = 0$ (fundamental mode) in this work.
