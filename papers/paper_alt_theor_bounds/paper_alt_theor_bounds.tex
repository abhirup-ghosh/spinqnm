...
change for either event when we include spin corrections to the EdGB QNM.
%
...
Let us contrast our results with those of Ref.~\cite{Lyu:2022gdr} which placed
the bound
%
$\ell_{\rm GB} \leqslant 1.33$~km
%
using the inspiral portion of the neutron star-black hole binaries GW200105 and GW200115~\cite{LIGOScientific:2021qlt} or
of Refs.~\cite{Nair:2019iur,Perkins:2021mhb}, which placed the bound $\ell_{\rm GB} \leqslant 1.7$~km, by considering
a selection of black hole binaries from the GWTC-1 and GWTC-1 catalogs~\cite{}\footnote{These bounds
are, strictly speaking, valid only when the scalar field $\varphi$ is small, i.e., $\varphi \ll 1$, and take into consideration only the leading-order
scalar field contribution arising from the dilatonic coupling.}
%



% which relies on post-Newtonian calculations of the GW phase shift in this theory~\cite{Yagi:2011xp,Shiralilou:2020gah,Shiralilou:2021mfl}
% combined with parametrized post-Einsteinian (ppE) framework~\cite{Yunes:2009ke,Chatziioannou:2012rf,Tahura:2019dgr}.


Based on the later, but now here confirmed, within the assumptions of our waveform model,
