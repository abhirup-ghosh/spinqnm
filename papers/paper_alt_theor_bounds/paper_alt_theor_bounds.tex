...
\begin{abstract}
% General:
\redsout{The detection of gravitational waves produced by coalescing compact binaries
allows}
\hs{Detections of gravitational waves from coalescing compact binaries allow}
us to explore \hs{into} the dynamical, nonlinear regime of general relativity
and \redsout{to} constrain modifications to it.
%
...
% Main question:
Can we use the ringdown to constrain deviations \redsout{to} \hs{from} general relativity,
...
%
We give a prescription on how to include in this waveform model\hs{,} the quasinormal mode frequencies
...
Einstein-dilaton-Gauss-Bonnet\hs{,} \redsout{and} dynamical Chern-Simons theories, and cubic- and quartic-order
effective-field-theories of general relativity.
%
% Main conclusion:
\redsout{We confront the resultant waveform models against the two loudest}
\hs{We use this parametrized waveform model to measure the ringdown properties of}
the two loudests \redsout{gravitational-wave} ringdown signals observed so far, \redsout{notably,} GW150914 and GW200129.
%
...
\redsout{during} \hs{in the post-merger}. So far, none of these null tests have reported any deviation from GR.
...
%
\redsout{to draw conclusions to any theory that predict\hs{s}
corrections at a given $p$.}
%
\redsout{This is a important difference between our work and Ref.~\cite{Carullo:2021dui}.
In particular, as we will see in Sec.~\ref{sec:results},}
...
\end{equation}
%
where $p_j(\ell_{\rm th} | \mathcal{H})$ are the priors used for each observation, \redsout{and} $p(\ell_{\rm th} | \mathcal{H})$ is an overall prior\hs{, and we assume that the value of $\ell_{\rm th}$ is shared among all events.}
%
...
\redsout{multiple} \hs{both} events to obtain the strongest possible bound on $\ell_{\rm th}$.
...
show how all other intrinsic \redsout{parameter} parameters remain unbiased.
...
%
\hs{We also thank Anuradha Gupta and Gregorio Carullo for comments on this work.}
%
