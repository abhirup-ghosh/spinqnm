...
%
\hscomm{I added the values of $p$ to the column headers to keep both columns with the same number of rows. I also
removed the subscript 220 because we already said we consider only this mode.}
}
...
%
\hscomm{I commented out the 68\% results for consistency with the other sections.}
%
...
% %
% \begin{equation}
% \ell_{\rm dCS} \leqslant 28.8~\textrm{km}
...
$\mathfrak{m} = M_{\rm f}$, $\varepsilon = 1/2$\hs{, and $N_{\rm max} = 0$.}
%
...
%
\hs{Having in mind that the cut off $\varepsilon = 1/2$ is not fundamental, but to keep consistency
...
% %
% \begin{equation}
%     \kappa \zeta = (\ell_{\rm dCS} / M^{\rm s})^4 = \gamma_{\rm dCS},
% \end{equation}
% %
...
%
\hscomm{Instead calling them ``$+$'' and ``$-$'' following the notation used in~\cite{Cano:2021myl},
I call them axial/polar to follow our conventions.}
%
...
inspiral and because the remnant BH parameters are estimated according to GR
