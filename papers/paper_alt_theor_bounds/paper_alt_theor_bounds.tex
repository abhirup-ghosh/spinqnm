...
% ---------------------------------------------------
% Template. Complete when we have the main text done.
% ---------------------------------------------------
% This paper is organized as follows.
% %
% In Sec.~\ref{sec:review_theories} we present a short overview of the modified gravity theories
% which we will consider and (if any) the current constraints on them.
% %
% In Sec.~\ref{sec:method} we review the two frameworks we combine in our work: the {\sc Parspec}
% parametrization for the QNMs and the {\sc pSEOBNR} waveform model. We also explain how the
% theory-specific QNM calculation can be mapped onto the theory-agnostic {\sc Parspec} framework.
% %
% In Sec.~\ref{sec:results} we explain how we carry ...
% %
% Finally, we present our conclusion in Sec.~\ref{sec:discussion} and point directions for future work.
% %
% Unless stated otherwise, we use geometrical unit $c = G = 1$.
...
We start by reviewing the parametrized ringdown spin expansion coefficients formalism (ParSpec)
introduced in Ref.~\cite{Maselli:2019mjd}, but following some of the notation of Ref.~\cite{Carullo:2021dui}.
%
The formalism modifies the Kerr quasinormal mode frequency $\omega_{J}^{\rm K}$ and
damping time $\tau_{J}^{\rm K}$ as
%
\begin{subequations}
...
\end{subequations}
%
where the $J$ collectively represents the QNM labels $\{l,\, m,\, n\}$, where $l$ is
the , $m$ and $n$ the ove

We expand the Kerr expression as:
%
\begin{subequations}
\label{eq:kerr_expansion}
\begin{align}
\omega_{J} &= \frac{1}{M} \sum_{n = 0}^{N_{\rm max}} \, \chi^{n} \omega^{(n)}_{J} \, \left( 1 + \gamma \delta \omega^{(n)}_{J} \right), \\
\tau_{J}   &= M     \sum_{n = 0}^{N_{\rm max}} \, \chi^{n} \tau^{(n)}_{J}   \, \left( 1 + \gamma \delta \tau^{(n)}_{J} \right),
\end{align}
\end{subequations}
%
where $M$ and $\chi$ are the detector-frame mass and dimensionless spin of the
remnant black hole; $\omega_{K}^{n}$, $\tau_{K}^{n}$ are dimensionless coefficients
...
\end{equation}
%
which depends on the parameter $\ell$, with dimensions of lenght, which sets the
lenghtscale in which beyond-GR modification become important and the exponent $p$
which is related to how the beyond-GR modifications are included to the Einstein-Hilbert action.
%
In Eq.~\eqref{eq:def_gamma} we made $\ell$ dimensionless by the other
lenghtscale associated to the remnant black hole, i.e., its source-frame mass
$M_{\rm s}$, which we can also in terms of the detector-frame mass $M$ through
a the redshift $z$.
...
%
This means we can identify the $M$ and $\chi$ with their corresponding GR values.
