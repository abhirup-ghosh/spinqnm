...
%
In Fig.~\ref{fig:edgb_exec_sum} we show the marginalized posteriors of the coupling constant $\ell_{\rm GB}$,
...
%
This does not mean that we are observing a deviation from GR. We recall that
the deviations from GR in the ParSpec framework are controlled by $\gamma$,
which here reads,
%
\begin{equation}
...
%
For GW150914, we see that for the $N_{\rm max} = 0$ that the CDF never goes
past $0.2$, regardless of the mass scale $\mathfrak{m}$, even at $\varepsilon =
...
change when we include spin corrections to the EdGB QNM.
%
Altogether, \emph{we conclude that we cannot constrain EdGB gravity with our
present model}.
%
These conclusions are summarized in Table~\ref{tab:summary_edgb}.
