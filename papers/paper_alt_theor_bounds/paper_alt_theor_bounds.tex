...
finite-size effects (see, e.g., Ref.~\cite{Sennett:2019bpc}). Thus, for each gravity theory the fundamental lenght-scale 
$\ell \lesssim GM/c^2$, where $M$ is the mass of the BH. This implies 
...
%
We hope that our results can serve as further motivation to address these
...
%
The resulting formula can then be expanded in $\gamma_{\rm EdGB}$ and the coefficients $\delta \omega^{(i)}$ and $\delta \tau^{(i)}$, $i=1,2$
...
%
These steps yield $p_{\rm EdGB} = 4$,
%
\begin{align}
...
%
This process, shows that the mode labeled ``-'' is the least damped and it is the one we use.
%
...
%
We then find this time that $p_{\rm qEFT} = 6$,
%
\begin{align}
