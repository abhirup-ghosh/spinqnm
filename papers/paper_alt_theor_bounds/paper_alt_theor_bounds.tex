...
Einstein-dilaton-Gauss-Bonnet and dynamical Chern-Simons theories, and cubic- and quartic-order
...
\hs{$\ell_{\rm EdGB} \leqslant 7.1$~km}
%
...
\hs{$\ell_{\rm EdGB} \leqslant 9.1$~km},
%
...
take into account only the leading-order scalar field interaction arising from the original dilatonic coupling, i.e.,
...
In this theory, NSs do not have scalar monopole charge~\cite{Yagi:2015oca}, while BHs do~\cite{Yunes:2011we,Sotiriou:2013qea,Sotiriou:2014pfa}. Finally, we take note constant $\alpha_{\rm EdGB}$ used in Refs.~\cite{Yagi:2011xp,Nair:2019iur,Perkins:2021mhb,Lyu:2022gdr} is related to $\ell_{\rm EdGB}$ as $\ell_{\rm EdGB} = 4 \pi^{1/4} |\alpha_{\rm EdGB}|^{1/2}$.
\hscomm{I had to translate between notations and rewrote the bounds quoted for $\alpha_{\rm EdGB}$ in terms of our $\ell_{\rm EdGB}$.}
}}.
...
\begin{align} \label{eq:action_dcs}
    ...
    \int \dV
    \left[
    \hs{\tfrac{1}{16\pi}} R - \tfrac{1}{2}(\partial \vartheta)^2
...
%
The Pontryagin density by itself, $\int \dV {}^{*}RR$, is a boundary
...
\hline
$\omega^{(0)}_{220}$    & 0.3737                    \\
$\tau^{(0)}_{220}$      & 11.2407                   \\
$\omega^{(1)}_{220}$    & 0.1258                    \\
$\tau^{(1)}_{220}$      & 0.2522                    \\
\hline
\hline
\end{tabular}
...
\hline
$\delta\omega^{(0)}_{220}$    & 0.0107                                               & 3.1964                   & $-$0.5813                     & $-$0.2114                       \\
$\delta\tau^{(0)}_{220}$      & 0.0044                                               & 6.3619                   & $-$0.2114                     & $-$0.6070                       \\
$\delta\omega^{(1)}_{220}$    & $-$0.2480                                            & 41.199                   & $-$3.8620                     & $-$1.5263                       \\
$\delta\tau^{(1)}_{220}$      & $-$1.1010                                            & 794.66                   & 265.12                        & 171.35                          \\
\hline
\hline
\end{tabular}
...
results using $\varepsilon = 1/2$, \hs{but we stress that there is no
fundamental justification for using this cut off.}
% %
...
% \abcomm{The reference to 80 km comes out of the blue. Doesn't it? Why did you refer to it?}~\hscomm{That was the upper prior limit for the GW150914 run. I modified the text to explain this.}
...
However, the result is robust under the hard cut-off of $\varepsilon = 1/2$ at 65\% credible level.}
}
...
% constant. What do you think?}~\hscomm{I agree. I introduced ``length-scale'' at a few other place in the text.}
...
%
\hs{The spin-corrections to the \emph{polar} gravitational-led modes
...
tells us in our ringdown analysis and the results of Ref.~\cite{Pierini:2021jxd} are our best guide
available presently.}
...
increase $\ell_{\rm dCS}$, at constant BH spin.
%
Hence, according to the prescription of Sec.~\ref{sec:theory_specific_qnm},
...
%
For the cubic EFT, we use their Eq.~(67), particularizing it to $\lame = \lamo = 1$.
%
We further linearize that equation in $\chi$ and consider the harmonic $m=2$.
%
This process, shows that the mode labeled ``-'' is the least-damped mode, and it is the one we use.
%
...
%
\hs{Indeed, since \pSEOB{} introduces only minimal modifications to the
inspiral and because the remnant BH parameters are estimated according GR
predictions, the fact that only small biases are introduced on $\Mf$ and $\chi_{\rm f}$
is, to some extend, expected.}
%
