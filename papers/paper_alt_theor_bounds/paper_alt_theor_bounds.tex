...
solutions.
%
In the first class, the first derivative of the coupling function $f'(\varphi) \equiv \dd f  / \dd \varphi$
...
related to the asymptotic $r^{-1}$ fall-off of the scalar field, \ab{where $r$ is the distance from the BH.}
%
...
\ab{the} smaller (larger) the BH mass is.
...
which affects the GW phase at $-1$PN order (relative to the dominant \ab{quadrupolar} GR
...
observations of compact binaries where at least \ab{if} one component is a BH.
%
...
%
by stacking the posteriors of $\ell_{\rm GB}$ from a selection of binary BHs (BBH)
...
between the QNM spectra of polar and axial gravitational \ab{perturbations~\cite{Glampedakis:2017rar,Lenzi:2021njy} 
...
perturbations couple to gravitational perturbations of axial parity, \ab{in contrast
with EdGB gravity.}
%
...
\end{equation}
%
where $\ell_{\rm EFT}$ is a length-scale assumed to be small compared to the \ab{length-scale $M$ 
...
Refs.~\cite{Cano:2020cao,Cano:2021myl} and \sout{work up} \ab{consider up} to dimension-eight operators
($n=4$),
%
\begin{subequations}
...
%
\ab{For simplicity,} in the dimension-six case we further assume that $\lame = \lamo = 1$, leaving us with $\ell = \ell_{\rm cEFT}$
as our single free parameter.
%
\ab{Similarly,} in the dimension-eight case, we set $\varepsilon_{1} = 1$ and $\varepsilon_{2} = \varepsilon_{3} = 0$,
...
luminosity distance $D_L$\abcomm{I changed to capital $d$ because later we call the
...
detector-frame remnant mass $(1 + z) \, M_{\rm f}$ for GW150914, using the same waveform model,
...
%
The bottom panel shows the joint posterior \ab{obtained by} combining both events.
%
...
%
\ab{We recall} that the deviations from GR in the ParSpec framework are controlled
by \ab{the dimensionless parameter} $\gamma$, which here reads,
%
\begin{equation}
...
displaces the posteriors distributions towards smaller values of $\ell_{\rm GB}$ (see Fig.~\ref{fig:edgb_exec_sum}), 
and larger values of $\gamma_{\rm GB}$ (see Fig.~\ref{fig:gamma_gb}).
...
in the main text, our analysis of these events fails to satisfy the EFT bound~\eqref{eq:eft_bound}. \ab{We mark 
...
horizontal lines mark the $90\%$ credible levels. \ab{Having set the maximum value 
of $\varepsilon$ to be 1/2, we see} that in no situation the curves pass
through the 0.90 lines. This means that no bound on $\ell_{\rm GB}$ can be placed 
...
%
\ab{For the case with $N_{\rm max} = 1$, we find that the ``EFT bound'' is satisfied 
only for $\varepsilon \approx 0.8$ and $\approx1$ for GW150914 aand GW200129, respectively. 
...
%
We see that EdGB provides an example of a theory in which, \ab{with current GW events,} the inspiral portion
of the signal \ab{can be} more constraining than the ringdown portion of the signal.
%
...
%
As with EdGB gravity (see Sec.~\ref{sec:results_edgb}) although the PDF of $\ell_{\rm CS}$
...
Finally, since both events individually lead to a bound on $\ell_{\rm CS}$
...
Ref.~\cite{Silva:2020acr}, but \ab{it}
%
(i) relies only on GW observations, and
%
(ii) suggests that a ringdown analysis can potentially place constraints on
theories \ab{that, with current GW events, can evade GR tests} using inspiral information alone, such as the
...
is $\ell_{\rm CS} \leqslant 38.7$~km \ab{at 90\% credible level}.
}
...
\subsection{Cubic Effective-FieXld-Theory of General Relativity}
...
We presented an \ab{unified framework to combine the ParSpec formalism} introduced
to model deviations to the GR QNMs~\cite{Maselli:2019mjd} with the \pSEOB~waveform model~\cite{Brito:2018rfr,Ghosh:2021mrv}.
%
We showed with concrete examples, \ab{how theory-specific QNM calculations of
...
%
We found, in particular that, \ab{within the interpretation of our non-GR theories as EFTs and the 
