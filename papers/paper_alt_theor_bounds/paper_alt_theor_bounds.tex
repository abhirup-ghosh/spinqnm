...
EdGB      & 4 & 3.1964 & 6.3619 & \dots & \dots &  \cite{Wagle:2021tam}   & $\ell_{\rm CS} \leqslant 8.5$~km~(EM+GW)~\cite{Silva:2020acr}  & $\ell_{\rm CS} \leqslant 34.5$~km \\
...
only parity-even corrections and the second entries by considering
only parity-odd corrections.
%
...
perturbative-conditions~\ref{eq:parspec_bound} imposed by the {\sc Parspec}
formalism are violated for the $\gamma_{\rm CS} \delta \tau^{(1)}$ coefficient.
%
This means that we cannot use these posterior to infer any meaningful bound on dCS gravity.
%
Now, since both event individually led to a bound on $\ell_{\rm CS}$ (assuming a cut-off scale for $\gm = M_{f}$
and $j = 0$), we can combine the posteriors to obtain the cumulative bound,
%
\begin{equation}
...
\hline
\hline
Event & $j$ & Eq.~\eqref{eq:eft_bound}? & Eq.~\eqref{eq:parspec_bound}?  & Bound ($\gm = M_{f}$) \\
\hline
