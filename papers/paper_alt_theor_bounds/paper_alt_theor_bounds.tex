...
%
Some of these gravitational-wave events are detected with sufficiently high
signal-to-noise ratio \ab{in the merger}, allowing us to probe the relaxation of the remnant black
hole to its final, stationary state --- the so-called black-hole ringdown,
...
augmented with a parametrization of the ringdown based on an expansion on the
...
%
% Main conclusion:
We confront the resultant waveform models against \ab{the two loudest gravitational-wave ringdown signals,
  notably, GW150914 and GW200129, and we find} that while the Einstein-dilaton-Gauss-Bonnet theory cannot be constrained with
...
\ab{Having reviewed the modified gravity theories we will consider, we now present
...
\ab{A general procedure to describe deviations to the QNM frequencies $\omega_{\ell m n}^{\rm Kerr}$ and
damping times $\tau_{\ell m n}^{\rm Kerr}$ of the GR Kerr BH is to write,}
%
\begin{subequations}
\label{eq:general_deviation}
\begin{align}
\omega_{\ell m n} &= \omega^{\rm Kerr}_{\ell m n} \, (1 + \delta \omega_{\ell m n}), \\
\tau_{\ell m n }   &= \tau^{\rm Kerr}_{\ell m  n}   \, (1 + \delta \tau_{\ell m n }),
\end{align}
\end{subequations}
%
\ab{where $\delta\omega_{\ell m n}$ and $\delta\tau_{\ell m  n}$ are the fractional deformation parameters,
$l$ and $m$ are the multipole indices, and $n$ the overtone number.}
%
...
of Ref.~\cite{LIGOScientific:2021sio}) take a restrictive theory-agnostic
...
\ab{The ParSpec framework was introduced in Ref.~\cite{Maselli:2019mjd}. It is} an observable-based
...
$\omega_{\ell m n}^{(j)}$ and $\tau_{\ell m n}^{(j)}$ are dimensionless coefficients of the
expansion in spin for Kerr QNMs; $\delta\omega_{\ell m n}^{(j)}$ and $\delta\tau_{\ell m n}^{(j)}$ are source-independent
...
\end{equation}
%
which depends on \ab{the length-scale parameter $\ell$ (non-GR modifications will become
important at distances $\lesssim \ell$) and 
...
\caption{(Color online).~Corner plot showing that the inferred final spin $\chi_f$,
detector-frame remnant mass $(1 + z) \, M_f$ for GW150914, using the same waveform model,
but without (blue contours)\agcomm{colors to be updated.} and with the non-GR parameters different from zero (orange contours, for
dCS gravity and $N_{\rm max}=0$). The contours represent 90\% credible levels.
%
We see that the introduction of the non-GR parameters does not bias the
inference on the source parameters.
%
\hs{We also need to fix overlapping tick labels.}
}
