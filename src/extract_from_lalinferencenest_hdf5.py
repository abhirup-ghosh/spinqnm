...
def get_samples_mcmc(filename, resume=False):
  """ 
  Returns a numpy record array with the samples.
  ...
  possamp = get_samples_mcmc(posfilename)
  print possamp.dtype.names
  mc_arr = possamp['mc']
  """
  fileobject = h5py.File(filename, 'r')
  sampletype, = fileobject.get('lalinference/lalinference_mcmc').keys()
  return fileobject.get('lalinference/lalinference_mcmc/%s'%(sampletype)).value
